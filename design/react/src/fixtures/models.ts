...
export const LOXAIC_MODELS: ModelInfo[] = [
...
export const LOXAIC_WORKSPACES: Workspace[] = [
...
export function getModelName(id: string): string {
  return LOXAIC_MODELS.find(m => m.id === id)?.display_name ?? id
}

export function getModelContext(id: string): number {
  return LOXAIC_MODELS.find(m => m.id === id)?.context_tokens ?? 32768
}

export function getModelPrice(id: string): number {
  return LOXAIC_MODELS.find(m => m.id === id)?.price ?? 0
}
